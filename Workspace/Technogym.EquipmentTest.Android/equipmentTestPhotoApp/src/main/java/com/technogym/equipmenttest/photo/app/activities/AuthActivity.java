...
import com.technogym.equipmenttest.photo.app.service.requests.GetPhotoVerificationActionsRequest;
import com.technogym.equipmenttest.photo.app.service.requests.IRequest;
import com.technogym.equipmenttest.photo.app.service.response.GetPhotoVerificationActionsResponse;
import com.technogym.equipmenttest.photo.app.tasks.ServiceTask;
...
//
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == IntentIntegrator.REQUEST_CODE) {
            ...
            Logger.getInstance().logDebug(this.getClass().getSimpleName(), "[onActivityResult] Result is null: " + (result != null));
            if (result != null) {
    ...
    private void startTest() {
        ...
        IRequest req = new GetPhotoVerificationActionsRequest(editTextSn.getText().toString());
        new ServiceTask(this, TargetServices.GET_PHOTO_VERIFICATION_ACTIONS_SERVICE, req).execute();
    ...
    public void onTaskComplete(Object data, TargetServices targetService) {
        ...
        GetPhotoVerificationActionsResponse response = (GetPhotoVerificationActionsResponse)data;
        List<VerifyImageModel> verifyImageModels = response.getVerifyImageModels();

        Log.i(this.getClass().getSimpleName(), "Actions IDs count: " + verifyImageModels.size());
        Logger.getInstance().logDebug(this.getClass().getSimpleName(), "Actions IDs count: " + verifyImageModels.size());

        // verifica presenza azioni
        if(verifyImageModels.size() == 0) {
            // errore, nessun azione
            AlertDialog alertDialog = new AlertDialog.Builder(this).create();
            alertDialog.setCancelable(false);
            alertDialog.setCanceledOnTouchOutside(false);
            alertDialog.setTitle(this.getResources().getString(R.string.error));
            alertDialog.setMessage(this.getResources().getString(R.string.no_actions_found));
        ...
        myIntent.putExtra(MainActivity.INTENT_KEY_VERIFY_IMAGE_MODELS_COUNT, verifyImageModels.size());
        for (int i = 0; i < verifyImageModels.size(); i++) {
            myIntent.putExtra(MainActivity.INTENT_KEY_VERIFY_IMAGE_MODELS_PREFIX + i, verifyImageModels.get(i));
        }
