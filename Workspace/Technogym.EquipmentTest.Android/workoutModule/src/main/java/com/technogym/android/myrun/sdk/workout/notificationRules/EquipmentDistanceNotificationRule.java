...
import com.technogym.android.myrun.sdk.workout.notifiers.EquipmentDistanceNotifier;

public class EquipmentDistanceNotificationRule extends NotificationRule<IWorkoutListener> implements
		INotificationRule<IWorkoutListener> {

	// Construction

	protected EquipmentDistanceNotificationRule() {
		super();
	}

	public static EquipmentDistanceNotificationRule create() {
		return new EquipmentDistanceNotificationRule();
	}
	...
	public Iterator<INotifier<IWorkoutListener>> getNotifiers(final String notifyMessage) {
		...
		final Integer distance = Integer.parseInt(statusData[10], 10);

		final List<INotifier<IWorkoutListener>> notifiers = new ArrayList<INotifier<IWorkoutListener>>();
		notifiers.add(EquipmentDistanceNotifier.create(distance));
