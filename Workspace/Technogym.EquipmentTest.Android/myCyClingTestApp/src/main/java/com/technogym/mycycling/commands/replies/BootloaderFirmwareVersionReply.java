 ...
 * BLVER command reply
 */
public class BootloaderFirmwareVersionReply extends ACommandReply {

    // { Public constants fields

    public final static String CMD_NAME = "BLVER";
     ...
     */
    private BootloaderFirmwareVersionReply(String name, boolean isInfoCommand, String reply) {
        ...
        String blVersion = mReplyValues.get(0).substring(mReplyValues.get(0).length() - 5);
        mReplyValues = new ArrayList<>();
        mReplyValues.add(blVersion);
    }
    ...
    public static ICommandReply create(String reply) {
        return (new BootloaderFirmwareVersionReply(CMD_NAME, false, reply));
    }
